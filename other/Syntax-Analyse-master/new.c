...
int judgeDeclAndFuncDef()
{
	...
	if(tok == CONST) return 1;
	else if(tok == VOID) return 2;
	else if(tok == INT){
...
past newType(int type)
{
	...
	switch (type)
	{
		case VOID: var -> strValue = "VOID"; break;
		case INT: var -> strValue = "INT"; break;
		case CONST: var -> strValue = "CONST"; break;
		default:break;
	}
	var -> left = NULL;
	var -> right = NULL;
...
past newExpr(int oper, past left, past right, past next)
{
	...
	switch (oper)
	{
		case LESSEQ: var->strValue = "<=";break;
		case GREATEQ: var->strValue = ">=";break;
		case NOTEQ: var->strValue = "!=";break;
		case EQ: var->strValue = "==";break;
		case AND: var->strValue = "&&";break;
		case OR: var->strValue = "||";break;
		default:var->ivalue = oper; var->strValue="@";break;
	}
	var->nodeType = "expr";
	// var->ivalue = oper;
	var->left = left;
...
void showAst(past node, int nest)
{
	...
	if(strcmp(node->nodeType, "intValue") == 0)
		...
		printf("%s %s\n", node->nodeType, node->strValue);
	else if(strcmp(node->nodeType, "funcDecl") == 0\
	     || strcmp(node->nodeType, "blockDecl") == 0)
		printf("%s .\n", node->nodeType);
	else if(strcmp(node->nodeType, "parameter") == 0)
		// printf("%s %s\n", node->nodeType, node->strValue ? node->strValue : "");
		if(node->strValue=="") printf("%s %d\n", node->nodeType, node->ivalue);
		else printf("%s %s\n", node->nodeType, node->strValue);
	else if(strcmp(node->nodeType, "type") == 0)
		printf("%s %s\n", node->nodeType, node->strValue);
	else if(strcmp(node->nodeType, "arrayDecl") == 0)
		printf("%s %s\n", node->nodeType, node->strValue);
	else if(strcmp(node->nodeType, "negativeExp") == 0)
		printf("%s %s\n", node->nodeType, node->strValue);
	else if(strcmp(node->nodeType, "AssignStmt") == 0\
	     || strcmp(node->nodeType, "IfStmt") == 0 \
	     || strcmp(node->nodeType, "WhileStmt") == 0\
		 || strcmp(node->nodeType, "BreakStmt") == 0\
		 || strcmp(node->nodeType, "ContinueStmt") == 0\
		 || strcmp(node->nodeType, "ReturnStmt") == 0\
		 || strcmp(node->nodeType, "EmptyStmt") == 0\
		 || strcmp(node->nodeType, "expStmt") == 0)
		printf("%s .\n", node->nodeType);
		...
		if(tok == INT){
			...
			if(tok == Ident){
				...
				if(tok == ';'){
					...
					root -> left = t;
					return root;
				}else{printf("An Error occurred in astConstDecl: no ; \n");}
			}else{printf("An Error occurred in astConstDecl\n");return NULL;}
		}else{
...
past astBType()
{
	...
	else printf("An Error occurred in astBType\n");
	return t;
}
...
past astConstDef()
{
	...
	if(tok == Ident){
		...
		if(tok == '['){
			...
			tmp = consExp;
			if(tok == ']') advance();
			else{printf("An Error occurred in astConstDef\n");return NULL;}
		}
		while(tok == '['){
			advance();
			past consExp = astConstExp();
...
past astConstInitVal()
{
	...
	if(tok == '{'){
		...
		if(list -> left!=NULL) return root;
		else{printf("An Error occurred in astConstInitVal\n");return NULL;}	
	}
}
...
past astVarDecl()
{
	...
	if(tok == INT){
		...
		if(tok == ';'){
			advance();
			return root;
		}else{printf("An Error occurred in astVarDecl  ; needed\n");return NULL;}
	}else{printf("An Error occurred in astVarDecl\n");return NULL;}
}
	...
	if(tok == VOID || tok == INT){
		...
		if(tok == Ident){
			...
			if(tok == '('){
				...
				root -> right = block;
				return root;
			}else{printf("An Error occurred in astFuncDef\n");return NULL;}	
		}else{printf("An Error occurred in astFuncDef\n");return NULL;}	
	}else{printf("An Error occurred in astFuncDef\n");return NULL;}	
}
...
past astFuncType()
{
	...
	else printf("An Error occurred in astFuncType\n");
	return t;
}
...
//					advance();
			...
			}else printf("An Error occurred in astStmt\n");
		}else printf("An Error occurred in astStmt\n");
	}
	else if(tok == WHILE)
	{
...
past astPrimaryExp()
{
	if(tok == '(')
	{
		...
		else printf("An Error occurred in astPrimaryExp\n");
	}
	else if(tok == '-'){
		char* s1 = strdup(yytext);
		int l1 = yyleng;
		...
		{
			advance();
			past minus = newAstNode2("-","negativeExp",astExp(),NULL,NULL);
			// past pri = astExp();
			if(tok == ')'){
				advance();
				return minus;
		...
		if(tok == '('){
			...
			if(tok == ')'){
				...
				if(tok == ')'){
					advance();
					return root;
				}else printf("An Error occurred in astUnaryExp\n");
			}
			return NULL;
		}
		//PrimaryExp.LVal
